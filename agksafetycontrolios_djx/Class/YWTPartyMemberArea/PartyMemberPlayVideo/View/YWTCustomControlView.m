...
@interface YWTCustomControlView ()<ZFSliderViewDelegate>
/// 底部工具栏
...
@implementation YWTCustomControlView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        [self.topToolView addSubview:self.fullScreenBackBtn];
        [self.topToolView addSubview:self.titleLabel];
        [self.bottomToolView addSubview:self.bottomPlayBtn];
...
- (void)makeSubViewsAction {
    ...
    [self.fullScreenBackBtn addTarget:self action:@selector(selectBackBtnAction:) forControlEvents:UIControlEventTouchUpInside];
    [self.bottomPlayBtn addTarget:self action:@selector(bottomPlayBtnAction:) forControlEvents:UIControlEventTouchUpInside];
}
...
-(void) bottomPlayBtnAction:(UIButton *)sender{
    [self playOrPause];
}
/// 根据当前播放状态取反
- (void)playOrPause {
...
- (void)playBtnSelectedState:(BOOL)selected {
    ...
    self.bottomPlayBtn.selected = selected;
}
...
- (void)layoutSubviews {
    ...
    min_y = 0;
    min_w = min_view_w;
    min_h = (iPhoneX && self.player.isFullScreen) ? 80 : 60;
    ...
    min_w = 30;
    min_h = 30;
    self.fullScreenBackBtn.frame = CGRectMake(min_x, min_y, min_w, min_h);
    self.fullScreenBackBtn.zf_centerY = self.topToolView.zf_centerY;
    
    min_x =self.fullScreenBackBtn.zf_right+5;
    min_y = 0;
    min_w = min_view_w - min_x - 15;
    min_h = 30;
    self.titleLabel.textColor = [UIColor colorTextWhiteColor];
    self.titleLabel.font = BFont(16);
    self.titleLabel.textAlignment = NSTextAlignmentCenter;
    self.titleLabel.frame = CGRectMake(min_x, min_y, min_w, min_h);
    self.titleLabel.zf_centerY = self.fullScreenBackBtn.zf_centerY;
    ...
    self.bottomPlayBtn.frame = CGRectMake(min_x, min_y, min_w, min_h);
    ...
    self.fullScreenBtn.zf_centerY = self.bottomPlayBtn.zf_centerY;
    ...
    self.totalTimeLabel.zf_centerY = self.bottomPlayBtn.zf_centerY;
    
    min_x = self.bottomPlayBtn.zf_right + 4;
    min_y = 0;
    min_w = self.totalTimeLabel.zf_left - min_x - 4;
    min_h = 30;
    self.slider.frame = CGRectMake(min_x, min_y, min_w, min_h);
    self.slider.zf_centerY = self.bottomPlayBtn.zf_centerY;
...
- (void)sliderValueChanged:(CGFloat)value currentTimeString:(NSString *)timeString {
    ...
    NSString *totalTimeString = [ZFUtilities convertTimeSecond:self.player.totalTime];
    self.totalTimeLabel.text = [NSString stringWithFormat:@"%@/%@",timeString,totalTimeString];
    self.slider.isdragging = YES;
...
-(UIButton *)fullScreenBackBtn{
    if (!_fullScreenBackBtn) {
        ...
        [_fullScreenBackBtn setImage:[UIImage imageNamed:@"rgzx_nav_ico_back"] forState:UIControlStateNormal];
...
- (UIButton *)playOrPauseBtn {
    if (!_playOrPauseBtn) {
        ...
        [_playOrPauseBtn setImage:[UIImage imageNamed:@"base_audio_timeOut"] forState:UIControlStateNormal];
        [_playOrPauseBtn setImage:[UIImage imageNamed:@"base_audio_paly"] forState:UIControlStateSelected];
    }
    return _playOrPauseBtn;
}

- (UIButton *)bottomPlayBtn{
    if (!_bottomPlayBtn) {
        _bottomPlayBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        [_bottomPlayBtn setImage:[UIImage imageNamed:@"base_audio_timeOut"] forState:UIControlStateNormal];
        [_bottomPlayBtn setImage:[UIImage imageNamed:@"base_audio_paly"] forState:UIControlStateSelected];
    }
    return _bottomPlayBtn;
}
...
- (ZFSliderView *)slider {
    if (!_slider) {
        ...
        _slider.maximumTrackTintColor = [UIColor colorWithHexString:@"#ff7c00"];
        _slider.bufferTrackTintColor  = [UIColor colorWithRed:1 green:1 blue:1 alpha:0.5];
        _slider.minimumTrackTintColor = [UIColor colorWithHexString:@"#ffa205"];
