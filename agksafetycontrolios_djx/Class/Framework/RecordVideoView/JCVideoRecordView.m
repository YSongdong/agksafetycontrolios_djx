...
- (void)showVedio:(NSURL *)playUrl{
    self.playView= [[JCRecordPlayerView alloc]initWithFrame:self.bounds];
    self.playView.backgroundColor = [UIColor clearColor];
    [_contentView addSubview:self.playView];
    self.playView.playUrl = playUrl;
    __weak typeof(self) instance = self;
    self.playView.cancelBlock = ^{
        [instance clickCancel];
    };
    self.playView.confirmBlock = ^{
        if (!instance.videoCompressComplete) {
            [instance showErrorWithTitle:@"正在压缩视频！" autoCloseTime:1];
            [instance showVedio:playUrl];
            return ;
        }
        [instance saveVideo];
...
- (void)compressVideo{
    ...
    [self.recorderManager compressVideo:self.recordVideoUrl complete:^(BOOL success, NSURL *outputUrl) {
        ...
        self.videoCompressComplete = YES;
    }];
}
#pragma mark - 取消录制的视频
- (void)clickCancel{
